# Adaptive design optimization using Non-contaminant model
# (code is not optimized and may take several days to run!)
# Results are saved in the file 'data/signaldt.Rdata'
rm(list=ls())
library(R.matlab)
...
# load data
load("~/Dropbox/signal_detection/data/memory.RData")
...
# list to save results from the analysis, data uses original response and stimulus sequences
sdt <- list()
...
for(aa in 1:ages){
  for(pp in 1:n.sub){
    for(tt in 1:times){
      ...
      summaries <- posterior.time(rs = y.1, st = x.1)
...
save(sdt,file = 'data/signaldt.RData')